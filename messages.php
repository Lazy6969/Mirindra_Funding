...
<div style="max-width: 800px; margin: 2rem auto; padding: 2rem; background: white; border-radius: 12px; box-shadow: var(--shadow-md);">
    <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #eee; padding-bottom: 1rem; margin-bottom: 1.5rem;">
        <div style="display: flex; align-items: center; gap: 1rem;">
            <a href="conversations.php" style="color: #666; text-decoration: none; font-size: 1.2rem; margin-right: 0.5rem;" title="Retour à la messagerie">
            ...
            <h2 style="margin: 0; font-size: 1.2rem;">Discussion avec <?php echo htmlspecialchars($receiver['first_name'] . ' ' . $receiver['last_name']); ?></h2>
        </div>
    </div>
        ...
        <?php foreach ($chat as $msg): ?>
            ...
            <div style="max-width: 70%; align-self: <?php echo $isMe ? 'flex-end' : 'flex-start'; ?>; background: <?php echo $isMe ? 'var(--primary-color)' : '#eee'; ?>; color: <?php echo $isMe ? 'white' : '#333'; ?>; padding: 0.8rem 1.2rem; border-radius: 15px; font-size: 0.95rem;">
                <?php if (!empty($msg['image_url'])): ?>
                    <img src="<?php echo SITE_URL . '/' . $msg['image_url']; ?>" style="max-width: 100%; height: auto; border-radius: 8px; margin-bottom: 8px; display: block;">
                <?php endif; ?>
    ...
    <form action="user-actions.php?action=send_message" method="POST" enctype="multipart/form-data" style="display: flex; gap: 10px; align-items: center;">
        ...
        <label for="message_image" class="btn btn-secondary" style="cursor: pointer; padding: 0.5rem 1rem; font-size: 1rem; min-width: auto; border-radius: 50%; width: 45px; height: 45px; display: flex; align-items: center; justify-content: center;" title="Envoyer une image">
